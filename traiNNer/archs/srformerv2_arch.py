...
class PSA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        kv = (
            ...
            .reshape(
                b_, n // 4, 2, self.num_heads, self.squeeze_dim * 4 // self.num_heads
            )
            .permute(2, 0, 3, 1, 4)
        )
        k, v = kv[0], kv[1]
        ...
        q = (
            self.q(x)
            .reshape(b_, n, 1, self.num_heads, self.squeeze_dim * 4 // self.num_heads)
            .permute(2, 0, 3, 1, 4)[0]
        )
        q = q * self.scale
...
def srformerv2(
    ...
    squeeze_dim: int = 60,
    img_range: float = 1,
    depths: Sequence = [4, 4, 4, 4, 4, 4],
    embed_dim: int = 240,
    num_heads: Sequence = [8, 8, 8, 8, 8, 8],
    mlp_ratio: int = 2,
