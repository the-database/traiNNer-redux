...
class CAB(nn.Module):
    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=30) -> None:
        super().__init__()
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, input_resolution, dim: int, norm_layer=nn.LayerNorm) -> None:
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
def drct_xl(
    ...
    window_size: int = 16,
    img_range: float = 1.0,
    depths: Sequence[int] = (6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6),
    embed_dim: int = 180,
    num_heads: Sequence[int] = (6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6),
    mlp_ratio: float = 2.0,
    upsampler: str = "pixelshuffle",
    resi_connection: str = "1conv",
) -> DRCT:
