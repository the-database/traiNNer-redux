    ...
    for k, v in state_dict.items():
        if "patch_embed.proj.weight" in k and len(v.shape) < 4:
            ...
            v = resize_pos_embed(
                v,
                model.pos_embed,
                0 if model.no_embed_class else getattr(model, "num_prefix_tokens", 1),
...
class PatchMerging(nn.Module):
    ...
    def __init__(
        self, input_resolution, dim, out_dim=None, norm_layer=nn.LayerNorm
    ) -> None:
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class SwinTransformer(nn.Module):
    ...
    def reset_classifier(self, num_classes, global_pool=None) -> None:
        self.num_classes = num_classes
