...
class SCC(nn.Module):
    ...
    def __init__(
        ...
        self.ngram_window_partition = NGramWindowPartition(
            dim, window_size, 2, num_heads, shift_size=0
        )
...
class NGramContext(nn.Module):
    ...
    def __init__(
        self, dim, window_size, ngram, ngram_num_heads, padding_mode="seq_refl_win_pad"
    ) -> None:
        super().__init__()
        _assert(
        ...
        self.ngram_attn = NGramWindowAttention(
            dim=dim // 2, num_heads=ngram_num_heads, window_size=(ngram, ngram)
        )
    ...
    def seq_refl_win_pad(self, x, back=False):
        ...
        if not back:
            (start_h, start_w), (end_h, end_w) = (
                to_2tuple(-2 * self.ngram + 1),
                to_2tuple(-self.ngram),
            )
            # pad lower
    ...
    def sliding_window_attention(self, unigram):
        ...
        slide = rearrange(
            slide, "b c h w ww hh -> b (h hh) (w ww) c"
        )  # [B, 2(wh+ngram-2), 2(ww+ngram-2), D/2]
        slide, num_windows = window_partition(
            slide, self.ngram
        )  # [B*wh*ww, ngram, ngram, D/2], (wh, ww)
        slide = slide.view(
            -1, self.ngram * self.ngram, self.dim // 2
        )  # [B*wh*ww, ngram*ngram, D/2]

        context = self.ngram_attn(slide).view(
            -1, self.ngram, self.ngram, self.dim // 2
        )  # [B*wh*ww, ngram, ngram, D/2]

        context = window_unpartition(
            context, num_windows
        )  # [B, wh*ngram, ww*ngram, D/2]
        context = rearrange(
            context, "b h w d -> b d h w"
        )  # [B, D/2, wh*ngram, ww*ngram]
    ...
    def forward(self, x):
        ...
        unigram = self.unigram_embed(x).view(
            B, D // 2, ph // self.window_size[0], pw // self.window_size[1]
        )

        unigram_forward_pad = self.seq_refl_win_pad(
            unigram, False
        )  # [B, D/2, wh+ngram-1, ww+ngram-1]
        unigram_backward_pad = self.seq_refl_win_pad(
            unigram, True
        )  # [B, D/2, wh+ngram-1, ww+ngram-1]

        context_forward = self.sliding_window_attention(
            unigram_forward_pad
        )  # [B, D/2, wh, ww]
        context_backward = self.sliding_window_attention(
            unigram_backward_pad
        )  # [B, D/2, wh, ww]

        context_bidirect = torch.cat(
            [context_forward, context_backward], dim=1
        )  # [B, D, wh, ww]
        context_bidirect = self.merge(context_bidirect)  # [B, D, wh, ww]
        context_bidirect = rearrange(
            context_bidirect, "b d h w -> b h w d"
        )  # [B, wh, ww, D]

        return (
            context_bidirect.unsqueeze(-2).unsqueeze(-2).contiguous()
        )  # [B, wh, ww, 1, 1, D]
...
class NGramWindowPartition(nn.Module):
    ...
    def __init__(self, dim, window_size, ngram, ngram_num_heads, shift_size=0) -> None:
        super().__init__()
        self.window_size = window_size[0]
        self.ngram = ngram
        self.shift_size = shift_size

        self.ngram_context = NGramContext(
            dim, window_size, ngram, ngram_num_heads, padding_mode="seq_refl_win_pad"
        )
    ...
    def forward(self, x):
        ...
        wh, ww = (
            ph // self.window_size,
            pw // self.window_size,
        )  # number of windows (height, width)
        _assert(
            0 not in [wh, ww], "feature map size should be larger than window size!"
        )
        ...
        if self.shift_size > 0:
            x = rearrange(
                windows, "b h w wh ww c -> b (h wh) (w ww) c"
            ).contiguous()  # [B, ph, pw, D]. re-patchfying
...
def hit_sng(
    ...
    in_chans: int = 3,
    img_size: int = 64,
    base_win_size: Sequence[int] = (8, 8),
    img_range: float = 1.0,
    ...
    hier_win_ratios: Sequence[float] = (0.5, 1, 2, 4, 6, 8),
