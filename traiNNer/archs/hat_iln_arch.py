# ruff: noqa
# type: ignore
# HAT with i-LN from https://arxiv.org/abs/2504.06629
...
class ChannelAttention(nn.Module):
    def __init__(self, num_feat, squeeze_factor: int | float = 16) -> None:
        super().__init__()
...
class HAB(nn.Module):
    """Hybrid Attention Block with i-LN."""
...
class OCAB(nn.Module):
    """Overlapping Cross-Attention Block with i-LN."""
    ...
    def forward(self, x, x_size, rpi):
        ...
        # i-LN: rescale by std from norm1
        x = std1 * self.proj(x) + shortcut

        # FFN with i-LN rescaling
        x_normed, std2 = self.norm2(x)
        x = x + std2 * self.mlp(x_normed)
        return x
...
class HAT_iLN(nn.Module):
    """Hybrid Attention Transformer with i-LN (Image Restoration Tailored Layer Normalization)."""
