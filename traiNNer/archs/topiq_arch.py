...
from .arch_util import dist_to_mos, load_pretrained_network, uniform_crop
...
class GatedConv(nn.Module):
    def __init__(self, weightdim, ksz=3) -> None:
        super().__init__()
...
class CFANet(nn.Module):
    ...
    def preprocess_face(self, x):
        ...
        if (
            ...
            raise AssertionError("No face detected in the input image.")

    def forward(
        self, x, y=None, return_mos=True, return_dist=False, align_crop_face=True
    ):
        if self.use_ref:
        ...
        if self.model_name == "topiq_nr_gfiqa_res50":
            if align_crop_face:
                ...
                x = nn.functional.interpolate(
                    x, size=(512, 512), mode="bicubic", align_corners=False
                )
