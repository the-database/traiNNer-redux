...
from . import cubic_spline, general, util
...
def analytical_base_partition_function(numer, denom):
    ...
    if not isinstance(numer, numbers.Integral):
        raise ValueError(f"Expected `numer` of type int, but is of type {type(numer)}")
    if not isinstance(denom, numbers.Integral):
        raise ValueError(f"Expected `denom` of type int, but is of type {type(denom)}")
    if not numer >= 0:
        raise ValueError(f"Expected `numer` >= 0, but is = {numer}")
    if not denom > 0:
        raise ValueError(f"Expected `denom` > 0, but is = {denom}")
