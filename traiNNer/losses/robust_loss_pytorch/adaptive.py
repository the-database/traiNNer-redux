...
from . import distribution, util, wavelet
...
class AdaptiveLossFunction(nn.Module):
    ...
    def __init__(
        ...
        if not np.isscalar(alpha_lo):
            raise ValueError(
                f"`alpha_lo` must be a scalar, but is of type {type(alpha_lo)}"
            )
        if not np.isscalar(alpha_hi):
            raise ValueError(
                f"`alpha_hi` must be a scalar, but is of type {type(alpha_hi)}"
            )
        if alpha_init is not None and not np.isscalar(alpha_init):
            raise ValueError(
                f"`alpha_init` must be None or a scalar, but is of type {type(alpha_init)}"
            )
        if not alpha_lo >= 0:
            raise ValueError(f"`alpha_lo` must be >= 0, but is {alpha_lo}")
        if not alpha_hi >= alpha_lo:
            raise ValueError(
                f"`alpha_hi` = {alpha_hi} must be >= `alpha_lo` = {alpha_lo}"
            )
        if alpha_init is not None and alpha_lo != alpha_hi:
            if not (alpha_init > alpha_lo and alpha_init < alpha_hi):
                raise ValueError(
                    f"`alpha_init` = {alpha_init} must be in (`alpha_lo`, `alpha_hi`) = ({alpha_lo} {alpha_hi})"
                )
        if not np.isscalar(scale_lo):
            raise ValueError(
                f"`scale_lo` must be a scalar, but is of type {type(scale_lo)}"
            )
        if not np.isscalar(scale_init):
            raise ValueError(
                f"`scale_init` must be a scalar, but is of type {type(scale_init)}"
            )
        if not scale_lo > 0:
            raise ValueError(f"`scale_lo` must be > 0, but is {scale_lo}")
        if not scale_init >= scale_lo:
            raise ValueError(
                f"`scale_init` = {scale_init} must be >= `scale_lo` = {scale_lo}"
            )
...
class StudentsTLossFunction(nn.Module):
    ...
    def __init__(
        self, num_dims, float_dtype, device, scale_lo=1e-5, scale_init=1.0
    ) -> None:
