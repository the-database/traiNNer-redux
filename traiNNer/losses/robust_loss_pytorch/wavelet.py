...
def generate_filters(wavelet_type=None):
    ...
    def mirror(f):
        return np.concatenate([f[-1:0:-1], f])
...
def pad_reflecting(x, padding_below, padding_above, axis):
    ...
    if not isinstance(padding_below, int):
        raise ValueError(
            f"Expected `padding_below` of type int, but is of type {type(padding_below)}"
        )
    if not isinstance(padding_above, int):
        raise ValueError(
            f"Expected `padding_above` of type int, but is of type {type(padding_above)}"
        )
    if not isinstance(axis, int):
        raise ValueError(f"Expected `axis` of type int, but is of type {type(axis)}")
    if not (axis >= 0 and axis < len(x.shape)):
        raise ValueError(f"Expected `axis` in [0, {len(x.shape) - 1}], but is = {axis}")
    x = torch.as_tensor(x)
...
def _check_resample_inputs(x, f, direction, shift) -> None:
    """Checks the inputs to _downsample() and _upsample()."""
    if len(x.shape) != 3:
        raise ValueError(f"Expected `x` to have rank 3, but is of size {x.shape}")
    if len(f.shape) != 1:
        raise ValueError(f"Expected `f` to have rank 1, but is of size {f.shape}")
    if direction not in (0, 1):
        raise ValueError(f"Expected `direction` to be 0 or 1, but is {direction}")
    if shift not in (0, 1):
        raise ValueError(f"Expected `shift` to be 0 or 1, but is {shift}")
...
def get_max_num_levels(sz):
    ...
    def log2(x):
        return np.log(np.float32(x)) / np.log(np.float32(2.0))
...
def construct(im, num_levels, wavelet_type):
    ...
    if len(im.shape) != 3:
        raise ValueError(
            f"Expected `im` to have a rank of 3, but is of size {im.shape}"
        )
...
def collapse(pyr, wavelet_type):
    ...
    if not isinstance(pyr, list | tuple):
        raise ValueError(f"Expected `pyr` to be a list or tuple, but is a {type(pyr)}")
    ...
    for d in range(num_levels - 1, -1, -1):
        if not isinstance(pyr[d], list | tuple):
            raise ValueError(
                f"Expected `pyr[{d}]` to be a list or tuple, but is a {type(pyr[d])}"
            )
        if len(pyr[d]) != 3:
            raise ValueError(
                f"Expected `pyr[{d}]` to have length 3, but has length {len(pyr[d])}"
            )
